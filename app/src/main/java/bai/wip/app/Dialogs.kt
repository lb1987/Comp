...
fun <T> Activity.optionRelatedDialog(
        title: String,
        xOptions: List<T>,
...
fun <T> Activity.optionUnRelatedDialog(
        title: String,
        xOption: List<T>,
        yOption: List<T>?,
        zOption: List<T>?,
    ...
    dialog(
        ...
        var xResult = xOption[0]
        var yResult = yOption?.get(0)
        var zResult = zOption?.get(0)
        with(view) {
            titleTV.text = title
            val adapter = OptionPicker.UnRelatedAdapter(xOption, yOption, zOption) { x, y, z ->
                xResult = x
                yResult = y
                zResult = z
            }
            optionPK.setUnrelatedAdapter(adapter)
            sureBTN.setOnClickListener {
                callback(xResult, yResult, zResult)
                dialog.dismiss()
            }
