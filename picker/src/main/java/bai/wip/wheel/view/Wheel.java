...
import bai.wip.wheel.timer.MessageHandler;
import bai.wip.wheel.timer.SmoothScrollTimerTask;
...
public class Wheel extends View {
    ...
    private int textSize = 17;//选项的文字大小
    ...
    public Wheel(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Wheel, 0, 0);
            mGravity = a.getInt(R.styleable.Wheel_gravity, Gravity.CENTER);
            textColorOut = a.getColor(R.styleable.Wheel_textColorSide, 0xFFa8a8a8);
            textColorCenter = a.getColor(R.styleable.Wheel_textColorCenter, 0xFF2a2a2a);
            dividerColor = a.getColor(R.styleable.Wheel_divColor, 0xFFd5d5d5);
            textSize = a.getDimensionPixelOffset(R.styleable.Wheel_textSize, textSize);
            lineSpacingMultiplier = a.getFloat(R.styleable.Wheel_lineSpaceFactor, lineSpacingMultiplier);
            a.recycle();//回收内存
        }
    ...
    private void initPaints() {
        ...
        paintIndicator.setStrokeWidth(mDividerHeight);
    ...
    public final void onItemSelected() {
        if (onItemChangedListener != null) {
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    onItemChangedListener.onChanged(getCurrentItem());
                }
            }, 200L);
        }
    }
